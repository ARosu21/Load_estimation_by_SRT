function [pdf_combi,range_pdf,cdf_combi,range_cdf] = dist_umbrales(srt,umbrales)
    ...
    for j=1:length(srt)
        ...
        if  srt(j) > 0 && srt(j) < umbral(1)
            idx(j)=1;
        elseif srt(j) >= umbral(1) && srt(j) < umbral(2)
            idx(j)=2;
        elseif srt(j) >= umbral(2) && srt(j) < umbral(3)
            idx(j)=3;
        else
            idx(j)=4;
        end
